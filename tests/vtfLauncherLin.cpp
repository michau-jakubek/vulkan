...
TriLogicInt runTest (add_ref<Canvas> ctx, add_cref<Params> params)
{
    ...
    const std::string lib =
        fs::path(cmd.getAppName()).replace_filename("libvtf.so").string();

    void* vtf = dlopen(lib.c_str(), RTLD_NOW);
    if (nullptr == vtf)
    {
        std::cout << "dlopen(\"" << lib << "\", RTLD_NOW) FAILED\n";
        return 1;
    }
    pVTF_GetAPI VTF_GetAPI = (pVTF_GetAPI)dlsym(vtf, VTF_GetAPI_PROC_NAME);
    if (nullptr == VTF_GetAPI)
    {
        std::cout << "dlsym(\"" << VTF_GetAPI_PROC_NAME << "\") FAILED\n";
        dlclose(vtf);
        return 1;
    }
    VTF_API* pVTF_API;
