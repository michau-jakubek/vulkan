                        ...
                        add_cref<std::vector<INodePtr>> list, uint32_t openSize)
{
    ...
    code << "layout(push_constant) uniform PC { float seed; } pc;\n";
    ...
    {
        generateLoops(list.back(), "root", code, 2, "counter.value[gl_LocalInvocationID.x]++");
    }
    INode::putIndent(code, 1);
    code << "}\n";
    code << "}\n";
...
TriLogicInt runTest (add_ref<VulkanContext> ctx, add_cref<Params> params)
{
    ...
    ZPipelineLayout			pipelineLayout  = lm.createPipelineLayout({ dsLayout }, ZPushRange<float>(stage));
    ...
    */

	{
        ...
        commandBufferPushConstants(cmd, pipelineLayout, testSeed);
